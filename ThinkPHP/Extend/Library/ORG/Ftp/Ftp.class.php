	...
	**/
    ...
    public function mkdir($dirname) {
        ...
        foreach ($dirname as $v) {
        	// miaomin edited @2014.3.12
            // if ($v && !$this->chdir($nowdir . $v)) {
        	if (!$this->chdir($nowdir . $v)) {
                if ($nowdir)
                    $this->chdir($nowdir);
    ...
    public function download($remotepath, $localpath, $mode = 'auto') {
    	...
    	$localdir= dirname($localpath); //获取本地文件夹
    	if(!is_dir($localdir)){
    		mkdir($localdir,0777,true);
    	}
    	$result = @ftp_get($this->link, $localpath, $remotepath, $this->mode);
    ...
    /*
     * 检测远程ftp上的的文件，如果存在返回1，如果没有建立远程文件夹
    */
    public function checkremotefile($remotefile){
    	
    	$remotefile_arr=$this->link->nlist($remotefile);
    	foreach($remotefile_arr as $key => $value){
    		if($remotefile==$value){
    			$result=1;
    		}else{
    			$result=0;
