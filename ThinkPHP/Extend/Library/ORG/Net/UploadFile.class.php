...
class UploadFile {//类定义开始
    ...
    private $config = array(
        ...
        'thumbType' =>0, // 缩略图生成方式 1 按设置大小截取 0 按原图等比例缩略  屏蔽后png会有透明通道
...
*/
    private function save($file) {
        ...
        //$filename=preg_replace ( '.jp', '/',$filename);
       // exit;
       // $imagea->imagezoom($filename,$filename."a",600,600,"#FFFFFF");
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
            	...
            	//$str=substr_replace($filename,"",strpos($filename,"."),strlen($filename));
            	//$filename_suffix=substr($filename,-(strlen($filename)-strpos($filename,".")),(strlen($filename)-strpos($filename,".")));//图片文件后缀
            	$filename_suffix=substr($filename,-4,4);//图片文件后缀     

            	//$tempstr=mb_strpos($filename,"'.'");
            	
            	if(strtolower($file['extension'])=='jpeg'){
            		$filename_suffix=substr($filename,-5,5);//图片文件后缀
            		$fname=substr($filename,0,strlen($filename)-5);
            		$mb_filename_6=$fname."_600".$filename_suffix;
            		$mb_filename_9=$fname."_960".$filename_suffix;
            	...
            	//copy($filename, $mb_filename); //拷贝文件
            	if($file['size']<1000000){
            		copy($filename, $mb_filename_6); 
            		copy($filename, $mb_filename_9);
            	}else{
            		$imagea->imagezoom($filename,$mb_filename_6,600,480,"#FFFFFF");
            		$imagea->imagezoom($filename,$mb_filename_9,960,768,"#FFFFFF");
            	}
                ...
                if(!is_dir($thumbPath)) {
                	...
                	if(is_dir(base64_decode($thumbPath))) {
                		...
                		if(!mkdir($thumbPath,0777,true)){
                			$this->error  =  '缩略图目录'.$thumbPath.'不存在';
                			return false;
                		}
                	}
                }else {
                	if(!is_writeable($thumbPath)) {
                		$this->error  =  '缩略图目录'.$thumbPath.'不可写';
                		return false;
                	}
                }
                // 生成图像缩略图
             ...
             // var_dump($filename);  以下$mb_filename_6为图片缩略后的600*600图,如果用原图缩略需要用 $filename
                    if(1 == $this->thumbType){
    ...
    public function uploadOne($file,$savePath=''){
        ...
        if(!empty($file['name'])) {
            ...
            foreach ($fileArray as $key=>$file){
                ...
                $file['filesize_disp'] = get_file_size($file['size']);
                $file['createdate'] = get_now();
                // 自动检查附件
                if($this->autoCheck) {
                    if(!$this->check($file))
                        return false;
            ...
            //$imagea=new ImageCheck();
            //$imagea->imagezoom("./upload/1.jpg","./upload/2.jpg",100,100,"#FFFFFF");
