...
$(document).ready(
	function () {
		swfu = new SWFUpload({
			...
			button_width: 100,
			button_height: 30,
			button_image_url: "images/upload_button.jpg",
		...
		function uploadStart(file) {
			var PostValueList = new Array('createtool', 'ctver', 'remark', 'subtool', 'subctver');
			for (var i = 0 ; i < PostValueList.length; i++)
			{ swfu.addPostParam(PostValueList[i], $('#fileinfo :input[name=' + PostValueList[i] + ']').val()); }
			swfu.addPostParam('isfree', $('#fileinfo :input[name=isfree]').attr("checked") ? 1 : 0);
		}
		...
		{ 
			...
			//$('#' + file.id).find('i').text('上传成功!'); setTimeout(function () { $('#' + file.id).fadeOut(500); }, 1000); 
		}
