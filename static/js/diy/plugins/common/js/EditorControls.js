...
THREE.EditorControls = function ( object, domElement, target) {
	...
	this.center = target === undefined ? new THREE.Vector3() : target;
	...
	this.rotate = function ( delta ) {
		...
		//new 
		// vector.copy(object.position).sub(center);
		// var object_matrix = object.matrixWorld;
		// delta.applyMatrix4(object_matrix);
		// //console.log(delta.x);
		// var radius = vector.length();
		// vector.x = delta.x;
		// vector.y = delta.y;
	...
	function onMouseMove( event ) {
		...
		if ( state === STATE.ROTATE ) {
			...
			//scope.rotate(new THREE.Vector3(-movementX * 0.05, movementY * 0.05, 0));
			...
			// zhengweifu 11/11/2014 禁止移动
			if (ZDIYCONFIGURE.FORBIDDENMOVEMENT) {
				console.log("禁止鼠标移动");
			} else {
